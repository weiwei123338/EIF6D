...
class TestDataset():
    ...
    def __getitem__(self, index):
        ...
        if self.setting == 'supervised' or self.setting == 'unsupervised_withMask':
            ...
            with open(os.path.join(self.data_dir, 'data', 'segmentation_results', 'test_trainedwoMask',
                                   path.split('/')[-1]), 'rb') as f:
        ...
        for j in range(num_instance):
            ...
            if len(choose) > 16:
                ...
                prior = self.mean_shapes[cat_id].astype(np.float32) #new
